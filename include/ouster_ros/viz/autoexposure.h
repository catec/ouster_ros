  ...
  public:
   ...
   void operator()(Eigen::Ref<Eigen::ArrayXd> key_eigen)
   {
      if (counter == 0)
      {
         ...
         std::nth_element(indices.begin(), indices.begin() + kth_extreme, indices.end(), cmp);
         lo = key_eigen[*(indices.begin() + kth_extreme)];
         std::nth_element(indices.begin() + kth_extreme, indices.end() - kth_extreme - 1, indices.end(), cmp);
