...
#include "ouster_ros/client/impl/client_impl.h"
...
int udp_data_socket(int port)
{
   ...
   if (!socket_valid(setsockopt(sock_fd, SOL_SOCKET, SO_RCVBUF, (char*)&RCVBUF_SIZE, sizeof(RCVBUF_SIZE))))
   {
...
std::shared_ptr<client> init_client(const std::string& hostname, int lidar_port, int imu_port)
{
   ...
   if (!socket_valid(cli->lidar_fd) || !socket_valid(cli->imu_fd)) return std::shared_ptr<client>();

   return cli;
}

std::shared_ptr<client> init_client(const std::string& hostname, const std::string& udp_dest_host, lidar_mode mode,
                                    timestamp_mode ts_mode, int lidar_port, int imu_port, int timeout_sec)
{
   ...
   if (!socket_valid(lidar_port) || !socket_valid(imu_port)) return std::shared_ptr<client>();
