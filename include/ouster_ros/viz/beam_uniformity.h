...
class BeamUniformityCorrector
{
  private:
    ...
    */
   static std::vector<double> compute_dark_count(const Eigen::Ref<const Eigen::ArrayXXd>& image)
   {
      ...
      Eigen::ArrayXXd row_diffs = image.bottomRows(image_h - 1) - image.topRows(image_h - 1);
      ...
      Eigen::Vector2d x = A.fullPivLu().solve(Eigen::Map<Eigen::VectorXd>(new_dark_count.data(), image_h, 1));

      Eigen::Map<Eigen::ArrayXd>(new_dark_count.data(), image_h, 1) -= (A * x).array();

      // subtract minimum value
      double min_el = *std::min_element(new_dark_count.begin(), new_dark_count.end());
   ...
   void update_dark_count(const Eigen::Ref<const Eigen::ArrayXXd>& image)
   {
      ...
      const size_t image_h          = image.rows();
      auto dark_count_map           = Eigen::Map<Eigen::ArrayXd>(dark_count.data(), image_h);
      const auto new_dark_count_map = Eigen::Map<const Eigen::ArrayXd>(new_dark_count.data(), image_h);
